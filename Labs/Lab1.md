...
7. In the `filebeat.yml` file, please set the `var.paths` under the Server logs section. It should looks like the following when you are done.
...
8.  Let's make sure nginx.yml is correct, `nano modules.d/nginx.yml`.  It should look like the following:
```
- module: nginx
  # Access logs
  access:
